...
set "SA3DIR="
if exist "..\stable-audio-3\.venv\Scripts\activate.bat" set "SA3DIR=..\stable-audio-3"
if exist "stable-audio-3\.venv\Scripts\activate.bat" set "SA3DIR=stable-audio-3"
...
if defined SA3DIR (
  ...
  echo      절을 따라 %USERPROFILE% 아래에 stable-audio-3 를 클론하고 uv sync 후
  echo      requirements-sa3.txt 를 설치하세요. 설치가 끝나면 run.bat 재실행으로
  echo      두 서버가 함께 켜집니다.
  echo      ^(찾은 경로 후보: ..\stable-audio-3\.venv 또는 .\stable-audio-3\.venv^)
)
echo.
